...
- **Relevant Spendly constraint**: Flag if the failure relates to known project rules (e.g., raw SQL f-strings instead of `?` placeholders, hardcoded URLs instead of `url_for()`, DB logic in routes instead of `db/db.py`, missing `PRAGMA foreign_keys = ON`)
...
- Route functions containing DB logic → must be in `db/db.py`
...
- `db/db.py` helpers assumed to exist before they are implemented → check step status in CLAUDE.md
